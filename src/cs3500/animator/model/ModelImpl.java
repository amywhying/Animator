          ...
          for (int i = 0; i < sortedShapeKeyFrames.size(); i++) {
            ...
            if (tick >= kf.getTime() && i + 1 < sortedShapeKeyFrames.size()) {
              ...
              double height = this.getKeyFrameTweening(startTime, endTime, tick, kf.getHeight(),
                      nextKf.getHeight());
              double x = this.getKeyFrameTweening(startTime, endTime, tick, kf.getX(),
                      nextKf.getX());
              double y = this.getKeyFrameTweening(startTime, endTime, tick, kf.getY(),
                      nextKf.getY());
              double red = this.getKeyFrameTweening(startTime, endTime, tick, kf.getRed(),
                      nextKf.getRed());
              double green = this.getKeyFrameTweening(startTime, endTime, tick, kf.getGreen(),
                      nextKf.getGreen());
              double blue = this.getKeyFrameTweening(startTime, endTime, tick, kf.getBlue(),
                      nextKf.getBlue());
              ...
              Color c = s.getColor();
              try {
                s.changeColor(red - c.getRed(), green - c.getGreen(),
                        blue - c.getBlue());
